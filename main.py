...
def list_users():
    ...
    user_list = [{"id": user.id, "username": user.username, "email": user.email, "phone": user.phone} for user in users]
...
def create_client():
    ...
    new_client = Client(name=data['name'], user_id=user_id, company_id=company_id, email=data['email'], phone=data['phone'])
